...
from server.models.game import ImportResult, RawGameExport
...
@router.get("/export", response_model=List[RawGameExport])
async def export_games(db=Depends(get_db)):
    return await export_raw_games(db)


@router.post("/import", response_model=ImportResult)
async def import_games(payload: List[RawGameExport], db=Depends(get_db)):
    return await import_raw_games(db, [entry.model_dump() for entry in payload])
